...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function initializeVideoControls(postElement) {
        ...
        playPauseBtn.addEventListener('click', () => {
            if (video.paused) {
                ...
                video.play().then(() => {
                    ...
                    loadingSpinner.style.display = 'none';
                    if (showToast) showToast('Error playing video.', '#dc3545');
        ...
        video.addEventListener('error', () => {
            if (showToast) showToast('Failed to load video.', '#dc3545');
            loadingSpinner.style.display = 'none';
        });
    ...
    // Listen for the custom event from post-interactions.js
    window.addEventListener('initVideoControls', () => {
        document.querySelectorAll('.post').forEach(postElement => {
            initializeVideoControls(postElement);
        });
    });

    // Expose initializeVideoControls if needed by other modules or for direct calls
