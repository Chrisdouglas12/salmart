...
// Constants for API and Socket URLs
...
// Listen for profile picture updates (this remains crucial for real-time UI)
...
// A tiny transparent GIF as a default placeholder to prevent broken image icons/layout shifts
...
function lazyLoadImage(imgElement, originalSrc) {
    // Set a placeholder immediately. This prevents a broken image icon before the IntersectionObserver fires.
    imgElement.src = DEFAULT_PLACEHOLDER_IMAGE;
    imgElement.classList.add('lazy-loading'); // Add class for CSS styling (e.g., blur, background color)

    const observer = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                // When in view, set the actual source
                const lazyImage = entry.target;
                lazyImage.src = originalSrc;
                lazyImage.onload = () => {
                    lazyImage.classList.remove('lazy-loading'); // Remove lazy loading styles
                    lazyImage.classList.add('loaded'); // Add a class for loaded state (e.g., for fade-in effect)
                    observer.unobserve(lazyImage); // Stop observing once loaded
                };
                lazyImage.onerror = () => {
                    lazyImage.src = '/salmart-192x192.png'; // Fallback if the image fails to load
        ...
        rootMargin: '0px 0px 200px 0px', // Load images when they are 200px from entering the viewport
...
function lazyLoadVideo(videoElement) {
    ...
    // Use a single IntersectionObserver for all videos for efficiency
    if (!window.videoIntersectionObserver) {
        window.videoIntersectionObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                const video = entry.target;
                if (entry.isIntersecting) {
                    // Load video sources when it enters the viewport
            ...
            rootMargin: '0px 0px 300px 0px', // Load videos when they are 300px from entering the viewport
            threshold: 0.5 // Start playing/loading when 50% visible
        });
    }

    videoElement.classList.add('lazy-loading'); // For initial styling (e.g., loading spinner)
    window.videoIntersectionObserver.observe(videoElement);
}


// --- UI Rendering Functions (Original Logic Preserved) ---
...
function renderUserSuggestion(user) {
    ...
    `;
    // Lazy load the user suggestion avatar
...
function renderPromotedPost(post) {
    ...
    const productImageForChat = post.photo || '/salmart-192x192.png'; // Original source for lazy loading

    // Use a placeholder image in the HTML, actual image set via lazyLoadImage
    mediaContent = `
        <img class="promoted-image" alt="Promoted Product" onerror="this.src='/salmart-192x192.png'">
    `;
    productDetails = `
    ...
    // Apply lazy loading to images within the promoted post
...
function renderPost(post) {
    ...
    // Apply lazy loading to images/videos and avatars within the post
    ...
    if (post.postType === 'video_ad') {
        ...
        if (videoElement) {
            ...
            if (window.initializeVideoControls) { // Ensure controls are initialized
...
// --- Main Post Fetching and Rendering Logic (Original Logic Preserved) ---
...
async function fetchPostsByCategory(category = currentCategory, page = currentPage, clearExisting = false) {
    ...
    isLoading = true;
    postsContainer.classList.add('loading'); // Indicate loading state

    if (clearExisting) {
        // Disconnect existing video observers before clearing content to prevent memory leaks
        if (window.videoIntersectionObserver) {
    ...
    try {
        ...
        // No longer need to collect video containers here, lazyLoadVideo handles observer internally

        for (let i = 0; i < nonPromotedPosts.length; i++) {
            const post = nonPromotedPosts[i];
            const postElement = renderPost(post); // This function now applies lazy loading internally
        ...
        postsContainer.appendChild(fragment); // Append all elements in one go

        if (postsContainer.children.length === 0) {
            postsContainer.innerHTML = '<p style="text-align: center; margin: 2rem;">No posts available.</p>';
        }

        window.dispatchEvent(new Event('postsRendered')); // Inform other modules that posts are rendered
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    // Centralized event delegation for clicks on dynamic content
    document.addEventListener('click', async (event) => {
        ...
        if (target.classList.contains('delete-post-button')) {
            ...
            if (confirm('Are you sure you want to delete this post?')) {
                try {
                    ...
                    showToast(error.message || 'Error deleting post.', '#dc3545');
                }
            }
            return;
        }
        ...
        if (target.classList.contains('edit-post-button')) {
            ...
            window.location.href = `edit-post.html?postId=${postId}&type=${postType}`;
            return;
        }
    });

    // Close post options menu when clicking anywhere else on the document
...
// Expose fetchPosts globally if needed by other parts of your app (e.g., category filters)
window.fetchPosts = fetchPostsByCategory;

// Cleanup on page unload (disconnect Socket.IO, unobserve videos)
